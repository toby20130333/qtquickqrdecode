...
DDQRDecoder::DDQRDecoder(QObject *parent) : QObject(parent)
{
...
DDQRDecoder::DDQRDecoder(DDQRDecoder::DecoderFormat decodeHints, QObject *parent) : QObject(parent)
{
    decoder = new MultiFormatReader();
    imageHandler = new ImageHandler();
...
DDQRDecoder::~DDQRDecoder()
{

}

void DDQRDecoder::setDecoder(const uint &hint)
{
...
QString DDQRDecoder::decodeImage(QImage image, int maxWidth, int maxHeight, bool smoothTransformation)
{
//    qDebug()<<" DDQRDecoder::decodeImage "<<image;
    QTime t;
    t.start();
    Ref<Result> res;
    ...
    if(image.isNull())
    {
        ...
        emit decodeimageerror(true);
        processingTime = -1;
//        qDebug()<<" image is NULL";
        return "";
    }else{
//        qDebug()<<" image is exist"<<image.size();
    }
...
QString DDQRDecoder::decodeImageFromFile(QString imageFilePath, int maxWidth, int maxHeight, bool smoothTransformation)
{
    ...
    return decodeImage(img, maxWidth, maxHeight, smoothTransformation);
}

QString DDQRDecoder::decodeImageQML(QObject *item)
{
    return decodeSubImageQML(item);
}

QString DDQRDecoder::decodeSubImageQML(QObject* item,
...
int DDQRDecoder::getProcessTimeOfLastDecoding()
{
    return processingTime;
}

uint DDQRDecoder::getEnabledFormats() const
{
